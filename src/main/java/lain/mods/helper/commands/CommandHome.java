...
public class CommandHome extends GeneralHelperCommand
{
            ...
            if (target != null)
            {
                ...
                else
                {
                    ...
                    if (target.teleportEntity(player))
                        sender.sendMessage(Message.msgHomeDone.convert(TextFormatting.GREEN));
                    else
                        sender.sendMessage(Message.msgTeleportationFailed.convert(TextFormatting.RED));
                }
            }
            else
                sender.sendMessage(Message.msgHomeNotFound.convert(TextFormatting.RED));
        }
        else
    ...
    {
        return Message.msgHomeUsage.key;
    }
