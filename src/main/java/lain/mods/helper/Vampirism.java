    ...
    public void tickStart(EnumSet<TickType> type, Object... tickData)
    {
        if (type.contains(TickType.PLAYER))
        {
            ...
            if (player.worldObj != null && !player.worldObj.isRemote && !player.capabilities.isCreativeMode)
            {
                ...
                {
                    boolean flag = LainHelper.sunlightDebuff && player.worldObj.isDaytime() && player.getBrightness(1.0F) > 0.5F && player.worldObj.canBlockSeeTheSky(MathHelper.floor_double(player.posX), MathHelper.floor_double(player.posY + player.getEyeHeight()), MathHelper.floor_double(player.posZ));
